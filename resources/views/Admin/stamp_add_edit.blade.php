...
<div class="container py-4">
    ...
    <form action="{{ route('stamp_add.update', [$stampHistory->customer_id, $stampHistory->stamp_id]) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <select name="customer_id[]" class="form-select select2" multiple required>
                @foreach($customers as $customer)
                    <option value="{{ $customer->id }}"
                        {{ in_array($customer->id, [$stampHistory->customer_id]) ? 'selected' : '' }}>
            ...
            <select name="stamp_id[]" class="form-select select2" multiple required>
                @foreach($stamps as $stamp)
                    <option value="{{ $stamp->id }}"
                        {{ in_array($stamp->id, [$stampHistory->stamp_id]) ? 'selected' : '' }}>
                        {{ $stamp->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="created_at" class="form-control"
                   value="{{ optional($stampHistory->created_at)->format('Y-m-d') }}">
        </div>
